...
*******************************************************************************
* DATA CLEANING  dofile. Note the this data is based on the IPUMS-DHS data*
* The dasets are freely available * 
*--------------------------------------------------
...
*AURTHOR: Rornald Muhumuza Kananura, London School of Economics, 
*--------------------------------------------------
...
set more off
log using Paper1_PhD.log, replace // Creating the run log file 

*Set directory to load the data - may be different depending on the disk  
...
*-------------------------------------------------------------------------------
**Generating Univariate tables 
*-------------------------------------------------------------------------------
global Singletone educlvl Cook_fuel_3 Cook_fuel Occup ARI TV Radio Treat_water Toilet_shared Cooking_Place Cook_fuel Roof_struc Floor_struc Wall_structures Occup Partner  ARI2 ARI1 ARI Fever Diarh_prev motorcycl carhh kidtwin kidsex hheadsex 

	foreach i in $descript { 
	tab `i' [iw= perweight] if year== xxxx, row missing 
	}
